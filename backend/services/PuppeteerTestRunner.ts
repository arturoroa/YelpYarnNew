...
// Define una interfaz para el resultado
...
export class SessionFilter1111Test {
  ...
  async execute(page: Page, config: TestConfig): Promise<void> {
    ...
    try {
      ...
      if (!page || !config.customPuppeteerCode) return;

      // Definir variable session aquí
      const session = {
        id: this.sessionId || 'test-session-id',
        guv: this.guv || 'test-guv-id'
      };

      this.logger.info(`Executing custom Puppeteer code for session: ${session.id}`);
      ...
      try {
        ...
        const customFunction = new Function(
          ...
          `
          return (async () => {
            ${config.customPuppeteerCode}
          })();
          `
        );

        // Execute the custom code with proper context
        await customFunction(
          ...
          session.guv,
          {
            log: (message: string) => this.logger.info(`Custom Code: ${message}`),
            error: (message: string) => this.logger.error(`Custom Code Error: ${message}`),
            warn: (message: string) => this.logger.warn(`Custom Code Warning: ${message}`)
          }
        );
        ...
        await this.database.addTestResult({
          ...
          scenario: 'custom_puppeteer_code',
          action: 'execute_custom_code',
          success: true,
          details: 'Custom Puppeteer code executed successfully',
          timestamp: new Date().toISOString()
        });

        this.logger.info(`Custom Puppeteer code executed successfully for session: ${session.id}`);

      } catch (error) {
        this.logger.error(`Custom Puppeteer code failed for session ${session.id}:`, error);
        ...
        await this.database.addTestResult({
          ...
          action: 'execute_custom_code',
          success: false,
          details: `Custom Puppeteer code failed: ${error.message}`,
          timestamp: new Date().toISOString()
        });

        // Don't throw error to prevent stopping other tests
      ...
      await this.database.addTestResult({
        ...
        action: 'navigate_home_screen',
        success: true,
        details: 'Loaded Yelp homepage (home_screen servlet)'
      });
      ...
      if (searchBox) {
        ...
        await this.database.addTestResult({
          ...
          success: true,
          details: 'Searched for "Acme Allopathic 2" (search → search_html servlets)'
        });
      }
      ...
      if (sponsoredResult) {
        ...
        await this.database.addTestResult({
          ...
          success: true,
          details: `Initial click on sponsored result at ${new Date(initialClickTime).toISOString()}`,
          click_recorded: true,
      ...
      await this.logSessionEvent('error', `Test 1 failed: ${error.message}`);
      throw error;
    }
  }

  async executeCustomCode(page: Page, config: TestConfig): Promise<void> {
    if (!page || !config.customPuppeteerCode) return;
    ...
    try {
      ...
      // Preparar el código para evaluación con la función de ayuda
      const customCode = `
      ...
      // Ejecutar código personalizado con tipo explícito
      const evalResult = await page.evaluate(customCode);
      // Usamos una variable intermedia con una aserción de tipo explícita
      const result = evalResult as CustomCodeResult;

      // Ahora usa result con su tipo correcto
      this.testLogger.logTestAction({
        type: 'custom_code',
        session_id: session.id,
        guv: session.guv,
      ...
      this.logger.error('Error executing custom code:', error);
      throw error;
    }
  }
